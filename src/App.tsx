...
import { ArrowLeft, Crown, Film, Heart, MonitorSmartphone, RotateCcw, UserRound } from 'lucide-react';
...
function Placeholder({ page, onBack }: { page: Exclude<Page, 'home' | 'reels'>; onBack: () => void }) {
  const map = {
    adult: { icon: Film, eyebrow: '18+ 专区', title: '专区筹备中', copy: '本期只保留一级入口，不展示具体内容、年龄验证或付费流程。' },
  ...
  return <main className={`placeholder-page ${page === 'adult' ? 'adult-page' : ''}`}><button className="back-button" onClick={onBack}><ArrowLeft size={19} />返回首页</button><div className="placeholder-mark"><Icon size={34} /></div><span className="eyebrow">{content.eyebrow}</span><h1>{content.title}</h1><p>{content.copy}</p><button className="primary-button" onClick={onBack}>回到首页</button></main>;
}
...
export function App() {
  ...
  const replay = () => { setReplayKey((v) => v + 1); setSplash(true); };
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      if (preview) setPreview(null); else if (searchOpen) setSearchOpen(false); else if (memberOpen) setMemberOpen(false); else if (page !== 'home') navigate('home');
    };
  ...
  return <div className="workspace" style={{ '--preview-width': `${previewWidth}px` } as CSSProperties}>
    <div className="app-frame">
      <div className="phone-surface">
        ...
        <SearchSheet open={searchOpen} items={dramas} onClose={() => setSearchOpen(false)} onOpen={(item) => { setSearchOpen(false); setPreview(item); }} />
