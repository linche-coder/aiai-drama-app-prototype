...
export function AuthSheet({ open, onClose, onPolicy, initialMode = 'login' }: { open: boolean; onClose: () => void; onPolicy: (kind: string) => void; initialMode?: 'login' | 'register' | 'forgot' }) {
  const { accept } = useApp();
  const [mode, setMode] = useState<'login' | 'register' | 'forgot'>('login'), [account, setAccount] = useState(''), [password, setPassword] = useState(''), [confirmPassword, setConfirmPassword] = useState(''), [email, setEmail] = useState(''), [agreed, setAgreed] = useState(false), [visible, setVisible] = useState(false), [busy, setBusy] = useState(false), [error, setError] = useState(''), [done, setDone] = useState(false);
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (mode === 'register' && !/^[A-Za-z0-9_]{4,20}$/.test(account.trim())) { setError('账号需为 4 至 20 位英文、数字或下划线。'); return; }
  ...
  function change(next: typeof mode) { setMode(next); setError(''); setDone(false); setPassword(''); setConfirmPassword(''); }
  return <Sheet open={open} title={title} onClose={() => { if (!busy) onClose(); }}><form className="auth-form" onSubmit={submit}>
    <div className="auth-brand"><img className="auth-logo" src={assetUrl('assets/brand/logo-full.svg')} alt="爱爱短剧" width="1000" height="301"/><p>{mode === 'forgot' ? '通过绑定邮箱重置密码' : '登录后管理你的追剧与评论'}</p></div>
    {mode !== 'forgot' && <><label>账号<input autoComplete="username" placeholder="请输入账号" value={account} minLength={mode === 'register' ? 4 : 1} maxLength={mode === 'register' ? 20 : 80} pattern={mode === 'register' ? '[A-Za-z0-9_]{4,20}' : undefined} onChange={e => setAccount(e.target.value)} required/><small className="muted">{mode === 'register' ? '4 至 20 位英文、数字或下划线' : '使用账号和密码登录'}</small></label><label>密码<span className="password-field"><input autoComplete={mode === 'register' ? 'new-password' : 'current-password'} placeholder={mode === 'register' ? '设置 8–64 位密码' : '请输入密码'} type={visible ? 'text' : 'password'} minLength={mode === 'register' ? 8 : 1} maxLength={64} value={password} onChange={e => setPassword(e.target.value)} required/><button type="button" aria-label={visible ? '隐藏密码' : '显示密码'} onClick={() => setVisible(!visible)}>{visible ? <EyeOff size={18}/> : <Eye size={18}/>}</button></span></label>{mode === 'register' && <label>确认密码<input type={visible ? 'text' : 'password'} autoComplete="new-password" minLength={8} maxLength={64} value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} placeholder="再次输入密码" required/></label>}</>}
    {mode !== 'login' && <label>{mode === 'register' ? '邮箱（选填）' : '已绑定邮箱'}<input type="email" autoComplete="email" placeholder="请输入邮箱地址" value={email} maxLength={100} onChange={e => setEmail(e.target.value)} required={mode === 'forgot'}/>{mode === 'register' && <small className="muted">未绑定邮箱时，无法通过邮箱找回密码。</small>}</label>}
