...
export const episodeCount = (d: Drama) => Math.max(1, Number(d.status.match(/\d+/)?.[0] ?? 1));
...
export function DramaDetailScreen({ item, onBack, onOpen, onPlay }: { item: Drama; onBack: () => void; onOpen: (d: Drama) => void; onPlay: (n: number) => void }) {
  ...
  const rights = useResource<{ preview_episode_ids?: string[] }>('/' + ((item.id.startsWith('adult-') || item.id.startsWith('private-')) ? 'adult' : 'green') + '/contents/' + enc(item.id));
  ...
  return <main className="reference-detail" aria-label={item.title + '详情'}>
    <div className="reference-detail-hero"><div className="detail-ambient" style={{ backgroundImage: 'url(' + item.cover + ')' }}/><button className="round-button detail-return" aria-label="返回" onClick={onBack}><ArrowLeft size={21}/></button>
      <div className="detail-summary"><img className="detail-poster" src={item.cover} alt={item.title}/><div><h1>{item.title}</h1><div className="detail-tags"><span>{item.genre}</span><span>{item.channel}</span><span>{item.region}</span></div><p>{item.heat} · 共 {episodeCount(item)} 集</p><button className="follow-pill" aria-pressed={records.following.includes(item.id)} onClick={() => toggle('following', item.id)}><Plus size={13}/>{records.following.includes(item.id) ? '已追剧' : '加入追剧'}</button></div></div>
    </div>
    <div className="reference-detail-body"><button className="watch-progress-card" onClick={() => onPlay(ep)}><span>{saved ? '上次看到 第 ' + ep + ' 集 · ' + percent + '%' : '开始观看 · 第 1 集'}<Play size={14}/></span><i><b style={{ width: percent + '%' }}/></i></button>
      ...
      <section className="similar-section"><h2>相似推荐</h2><div className="poster-rail">{related.map(d => <button className="rail-card" key={d.id} onClick={() => onOpen(d)}><span className="rail-cover"><img src={d.cover} alt="" loading="lazy"/></span><strong>{d.title}</strong><small>{d.status} · {d.heat}</small></button>)}</div></section>
    </div>
  </main>;
}
export function PlayerScreen({ item, episode, onBack, onEpisode, onMember }: { item: Drama; episode: number; onBack: () => void; onEpisode: (n: number) => void; onMember: () => void }) {
  ...
  useEffect(() => {
    ...
    const zone = (item.id.startsWith('adult-') || item.id.startsWith('private-')) ? 'adult' : 'green';
  ...
  return <main ref={container} className="reference-player player-refined" aria-label="短剧播放">
    ...
    <aside className="player-social">{([{ key: 'liked', icon: Heart, label: '喜欢' }, { key: 'comment', icon: MessageCircle, label: '评论' }, { key: 'saved', icon: Bookmark, label: '收藏' }, { key: 'share', icon: Share2, label: '分享' }] as const).map(({ key, icon: Icon, label }) => <button key={key} aria-label={label} aria-pressed={key === 'liked' || key === 'saved' ? records[key].includes(item.id) : undefined} onClick={() => key === 'comment' ? setPanel('comments') : key === 'share' ? void share() : toggle(key, item.id)}><span><Icon size={28} fill={(key === 'liked' || key === 'saved') && records[key].includes(item.id) ? 'currentColor' : 'none'}/></span><small>{label}</small></button>)}</aside>
