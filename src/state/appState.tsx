...
function readRecords(key: string): Records {
  ...
  return { following: ids(v?.following), saved: ids(v?.saved), liked: ids(v?.liked), history };
}
type State = { session: Session | null; checking: boolean; accept: (s: Session | null) => void; records: Records; toggle: (kind: 'following' | 'saved' | 'liked', id: string) => void; saveProgress: (p: Progress) => void; clearHistory: (id?: string) => void; login: () => void };
...
export function AppState({ children, login }: { children: ReactNode; login: () => void }) {
  ...
  const toggle: State['toggle'] = (kind, id) => update(r => ({ ...r, [kind]: r[kind].includes(id) ? r[kind].filter(v => v !== id) : [...r[kind], id] }));
  ...
  return <Context.Provider value={{ session, checking, accept, records, toggle, saveProgress, clearHistory, login }}>{children}</Context.Provider>;
}
